  ...
  list[i].onclick = function(e) {
    ...
    for (let i = 0; i < 4; i++) {
      window.setTimeout(function() {
        $(".wave-bottom").children().eq(i).addClass("moveup-floor");
      }, i * 100)
    }
...
function animate() {
  ...
  if (elapsed > fpsInterval) {
    ...
    $("use").eq(0).attr("fill", `hsla(${circleCollection[0].hue}, 100%, 60%, 0.7)`)
    $("use").eq(1).attr("fill", `hsla(${circleCollection[0].hue}, 100%, 60%, 0.5)`)
    $("use").eq(2).attr("fill", `hsla(${circleCollection[0].hue}, 100%, 60%, 0.3)`)
